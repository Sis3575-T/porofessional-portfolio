...
import { fingerprintService } from "shared/services/fingerprintService";
import { geolocationService } from "shared/services/geolocationService";
import { authenticateToken, requireAdmin, rateLimiter } from "shared/middleware";
...
export function setupSocketIO(app) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("leave-visitor-room", (visitorId) => {
      socket.leave(visitorId);
      console.log(`Visitor ${visitorId} left room`);
    });
...
function isBot(ua) {
  ...
  const botPatterns = [
    ...
    /dotbot/i, /rogerbot/i, /linkedinbot/i, /embedly/i, /quoralinkpreview/i,
    ...
    /bitrix linkpreview/i, /xing-contenttabreceiver/i, /chrome-lighthouse/i,
...
router.post("/track", async (req, res) => {
  try {
    const { visitorId, page, event, data, timezone, referrer } = req.body;
    if (!visitorId || !page) {
    ...
    if (!visitor) {
      visitor = await prisma.visitor.upsert({
        ...
        create: {
          ...
          timezone: location.timezone || timezone || null,
        ...
        update: {
          ...
          timezone: location.timezone || timezone || undefined,
          referrer,
        },
      });
    ...
    await prisma.visitor.update({
      ...
      data: {
        ...
        timezone: location.timezone || timezone || visitor.timezone,
        browser, os, deviceType, language,
...
router.post("/session/start", async (req, res) => {
  try {
    ...
    if (isNewSession) {
      ...
      if (currentVisit && !currentVisit.endedAt) {
        await prisma.visit.update({
          where: { id: currentVisit.id },
          data: { startedAt: now },
        });
