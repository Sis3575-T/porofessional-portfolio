  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={fetch} className="p-2 text-slate-400 hover:text-cyan-400 transition"><RefreshCw size={18} /></button>
      </div>
        ...
        <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
          {messages.map(msg => (
            <div key={msg.id}
              ...
              className={`cursor-pointer bg-slate-900/50 border rounded-xl p-4 transition hover:border-cyan-500/30 ${selected?.id === msg.id ? 'border-cyan-500/50' : 'border-slate-800'} ${!msg.isRead ? 'border-l-cyan-500 border-l-4' : ''}`}
            >
              <div className="flex items-start justify-between mb-1">
                <h4 className="font-medium text-white text-sm">{msg.name}</h4>
                <span className={`text-xs px-2 py-0.5 rounded-full ${msg.isRead ? 'text-slate-500' : 'text-cyan-400 bg-cyan-500/10'}`}>
                  {msg.isRead ? 'Read' : 'New'}
                </span>
              </div>
              <p className="text-xs text-slate-400 mb-1">{msg.subject}</p>
          ...
          {selected ? (
            <div className="space-y-4">
              ...
              <div className="p-3 bg-slate-800/30 rounded-lg">
                <p className="text-sm text-slate-300 font-medium">Subject: {selected.subject}</p>
              </div>
              <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap">{selected.message}</p>
              <div className="flex gap-3 pt-4 border-t border-slate-800">
                <button onClick={() => toggleRead(selected)} className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-lg text-sm text-slate-300 hover:text-white transition">
