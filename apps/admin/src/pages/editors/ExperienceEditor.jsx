    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="max-w-3xl mb-8 p-6 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
          <h3 className="font-semibold text-slate-900">{editing ? 'Edit' : 'New'} Experience</h3>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm text-slate-500 mb-1">Company URL</label>
              <input value={form.companyUrl} onChange={(e) => setForm({...form, companyUrl: e.target.value})} className="w-full" placeholder="https://..." />
            </div>
          </div>

          <div>
            <label className="block text-sm text-slate-500 mb-1">Technologies (comma separated)</label>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-slate-500 mb-1">Responsibilities (one per line)</label>
            ...
            <div>
              <label className="block text-sm text-slate-500 mb-1">Achievements (one per line)</label>
          ...
          <div>
            <label className="block text-sm text-slate-500 mb-1">Company Logo</label>
            <div className="flex gap-2">
              <input value={form.logo} onChange={(e) => setForm({...form, logo: e.target.value})} className="w-full" placeholder="https://..." />
              <button type="button" onClick={() => setPickerField('logo')} className="px-3 py-2 bg-slate-200 hover:bg-slate-200 text-slate-600 rounded-lg transition shrink-0">
                <ImageIcon size={16} />
              </button>
            </div>
          ...
          <div>
            <label className="block text-sm text-slate-500 mb-1">Gallery Images</label>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addGalleryImage} className="px-3 py-2 bg-slate-200 hover:bg-slate-200 text-slate-600 rounded-lg transition shrink-0 text-sm">Add</button>
            </div>
          ...
          <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-xl p-5 group">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-medium text-slate-900">{item.position}</h4>
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                <button onClick={() => handleEdit(item)} className="p-1 text-slate-500 hover:text-cyan-400"><Pencil size={14} /></button>
                <button onClick={() => handleDelete(item.id)} className="p-1 text-slate-500 hover:text-red-400"><Trash2 size={14} /></button>
              </div>
            </div>
          </div>
        ))}
      </div>
      {items.length === 0 && <p className="text-slate-400">No experiences added yet</p>}
