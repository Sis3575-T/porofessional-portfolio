...
export default function VisitorProfile() {
  ...
  const handleDelete = async () => {
    if (!confirm("Delete this visitor and all their data?")) return;
    try {
      ...
      <div className="grid md:grid-cols-4 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-1">
            ...
            {visitor.latitude && (
              <InfoRow icon={MapPin} label="Coordinates" value={`${visitor.latitude}, ${visitor.longitude}`} />
            )}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-1">
            ...
            <InfoRow icon={Monitor} label="Screen" value={visitor.screenResolution} />
            {visitor.ipHash && (
              <InfoRow icon={Hash} label="IP Hash" value={visitor.ipHash.slice(0, 16) + "..."} />
            )}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {[
              { label: "Skills Section", value: visitor.skillsViewed },
              { label: "Services Section", value: visitor.servicesViewed },
              { label: "Experience Section", value: visitor.experienceViewed },
              { label: "Education Section", value: visitor.educationViewed },
              { label: "Contact Section", value: visitor.contactViewed },
            ].map((item) => (
              <div key={item.label} className="flex items-center justify-between py-2 border-b border-slate-100">
                <span className="text-sm text-slate-600">{item.label}</span>
                <span className={`text-sm font-medium ${item.value ? "text-green-600" : "text-slate-400"}`}>
                  {item.value ? "Viewed" : "Not viewed"}
                </span>
              </div>
            ))}
              ...
              return (
                <div key={visit.id} className="border-l-2 border-cyan-400 pl-4 pb-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {visit.endedAt && (
                      <span className="text-xs text-slate-400">- {formatDateTime(visit.endedAt)}</span>
                    )}
                  </div>
